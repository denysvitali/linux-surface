...
{
	if (spx_route_added) {
		snd_soc_dapm_del_routes(&spx_component->dapm, spx_echo_routes,
					ARRAY_SIZE(spx_echo_routes));
		snd_soc_dapm_sync(&spx_component->dapm);
...
static int __init spx_wcd_echo_init(void)
{
	...
	if (enable) {
		...
		ret = snd_soc_dapm_add_routes(&spx_component->dapm,
					      spx_echo_routes,
					      ARRAY_SIZE(spx_echo_routes));
		if (ret)
			goto out_remove;
		...
		ret = snd_soc_dapm_sync(&spx_component->dapm);
		if (ret)
			goto out_remove;
	}
