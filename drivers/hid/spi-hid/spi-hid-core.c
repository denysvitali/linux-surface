...
static int spi_hid_power_down(struct spi_hid *shid)
{
	...
	/* Make any completion racing the shutdown discard its input. */
	shid->powered = false;
	spin_unlock_irqrestore(&shid->input_lock, flags);
...
/*
 * The EC normally announces the end of reset with an interrupt.  Keep a
 * fallback because that edge can occur before the interrupt controller has
 * finished unmasking the line during early boot.
 */
#define SPI_HID_STARTUP_FALLBACK_MS	500
...
static void spi_hid_startup_work(struct work_struct *work)
{
	struct spi_hid *shid = container_of(to_delayed_work(work),
						 struct spi_hid, startup_work);
	struct device *dev = &shid->spi->dev;
	unsigned long flags;
	int ret;

	/* The IRQ path already completed startup. */
	if (shid->ready || shid->power_state == SPI_HID_POWER_MODE_OFF)
		return;

	/*
	 * A missed edge leaves the reset response at the head of the device's
	 * input queue.  Drain it exactly as the IRQ handler would; processing
	 * that response will schedule reset_work and then request the descriptor.
	 */
	dev_warn(dev, "reset-complete IRQ missed, draining pending response\n");
	spin_lock_irqsave(&shid->input_lock, flags);
	ret = spi_hid_bus_input_report(shid);
	spin_unlock_irqrestore(&shid->input_lock, flags);
	if (ret) {
		dev_err(dev, "failed to drain startup response: %d\n", ret);
		...
		struct spi_hid_input_buf *buf)
{
	...
	case SPI_HID_REPORT_TYPE_DEVICE_DESC:
		...
		cancel_delayed_work(&shid->startup_work);
...
static int spi_hid_create_device(struct spi_hid *shid)
{
	...
	shid->creating_device = true;
	ret = hid_add_device(hid);
	shid->creating_device = false;
	if (ret) {
...
static void spi_hid_create_device_work(struct work_struct *work)
{
	...
	/*
	 * hid_add_device() publishes the input nodes and may open the transport
	 * before it returns.  Serialize with that open and do not undo it by
	 * powering the EC back down underneath an active input client.
	 */
	mutex_lock(&shid->hid->ll_open_lock);
	if (shid->hid->ll_open_count) {
		mutex_unlock(&shid->hid->ll_open_lock);
		return;
	}
...
static int spi_hid_ll_open(struct hid_device *hid)
{
	...
	struct device *controller_parent = spi->controller->dev.parent;
	u8 prev_state = shid->power_state;
	int ret;

	/*
	 * The SPI core runtime-resumes the controller for each individual
	 * message and autosuspends it shortly afterwards.  That is fine for
	 * register-oriented devices, but an interrupt-driven touchscreen can
	 * accumulate several reports while the controller wakes on the first
	 * contact.  Keep the controller awake for the lifetime of the HID open
	 * so input reports can be drained immediately.  The last HID close
	 * releases the reference and restores the controller's normal
	 * autosuspend behaviour.
	 */
	if (!shid->controller_pm_held) {
		ret = pm_runtime_resume_and_get(controller_parent);
		if (ret < 0) {
			dev_err(dev, "%s: failed to resume SPI controller: %d\n",
					__func__, ret);
			return ret;
		}
		shid->controller_pm_held = true;
	}

	if (shid->refresh_in_progress || shid->creating_device ||
	    (prev_state == SPI_HID_POWER_MODE_ACTIVE && shid->ready))
		return 0;
	...
	mod_delayed_work(system_wq, &shid->startup_work,
			 msecs_to_jiffies(SPI_HID_STARTUP_FALLBACK_MS));
	ret = wait_for_completion_interruptible_timeout(&shid->ready_done,
						 msecs_to_jiffies(2000));
	if (ret <= 0) {
		if (!ret)
			ret = -ETIMEDOUT;
		dev_err(dev, "%s: firmware did not become ready: %d\n",
				__func__, ret);
		goto err2;
	}
...
err0:
	if (shid->controller_pm_held) {
		pm_runtime_mark_last_busy(controller_parent);
		pm_runtime_put_autosuspend(controller_parent);
		shid->controller_pm_held = false;
	}
	return ret;
}
	...
	*/
	...
	if (ret)
		...
		shid->report_descriptor_crc32 = crc32_le(0,
					...
					len);
		/* The report-descriptor response means firmware is command-ready. */
		shid->ready = true;
		complete_all(&shid->ready_done);
		cancel_delayed_work(&shid->startup_work);
		sysfs_notify(&dev->kobj, NULL, "ready");
...
static int spi_hid_probe(struct spi_device *spi)
{
	...
	*/
	...
	INIT_DELAYED_WORK(&shid->startup_work, spi_hid_startup_work);
	 ...
	 */
	msleep(150);
	schedule_delayed_work(&shid->startup_work,
			      msecs_to_jiffies(SPI_HID_STARTUP_FALLBACK_MS));
...
static void spi_hid_remove(struct spi_device *spi)
{
	...
	cancel_delayed_work_sync(&shid->startup_work);
	spi_hid_power_down(shid);
