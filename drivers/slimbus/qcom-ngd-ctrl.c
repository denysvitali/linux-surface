				     ...
				     struct qcom_slim_ngd_ctrl *ctrl)
{
	...
	for_each_available_child_of_node(parent->of_node, node) {
		...
		if (spx_ngd_base_offset >= 0) {
			if (spx_ngd_base_offset != 0 &&
			    spx_ngd_base_offset != data->offset) {
				dev_err(parent,
					"SPX: refusing unsafe NGD base offset 0x%x; only 0x0/0x%x are reload-safe\n",
					spx_ngd_base_offset, data->offset);
			...
			ngd->base = ctrl->base + spx_ngd_base_offset;
		} else {
			ngd->base = ctrl->base + ngd->id * data->offset +
						(ngd->id - 1) * data->size;
			}
			dev_info(parent, "SPX: NGD child id=%u base_offset=0x%lx\n",
				 ngd->id, (unsigned long)(ngd->base - ctrl->base));

			dev_info(parent, "SPX: driver_set_override begin for NGD child id=%u\n",
				 ngd->id);
			...
			if (ret) {
				dev_err(parent,
					"SPX: driver_set_override failed for NGD child id=%u ret=%d\n",
					ngd->id, ret);
				platform_device_put(ngd->pdev);
				kfree(ngd);
				of_node_put(node);
			...
			dev_info(parent, "SPX: driver_set_override done for NGD child id=%u\n",
				 ngd->id);
			ngd->pdev->dev.of_node = node;
			ctrl->ngd = ngd;

			dev_info(parent, "SPX: platform_device_add begin for NGD child id=%u\n",
				 ngd->id);
			ret = platform_device_add(ngd->pdev);
			if (ret) {
				dev_err(parent,
					"SPX: platform_device_add failed for NGD child id=%u ret=%d\n",
					ngd->id, ret);
				platform_device_put(ngd->pdev);
				kfree(ngd);
				of_node_put(node);
			...
			dev_info(parent, "SPX: platform_device_add done for NGD child id=%u\n",
				 ngd->id);
...
static int qcom_slim_ngd_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "SPX: child probe: workqueue init begin\n");
	...
	dev_info(dev, "SPX: child probe: workqueue init done\n");
...
{
	struct qcom_slim_ngd_ctrl *ctrl = dev_get_drvdata(&pdev->dev);

	ctrl->removing = true;

	/*
	 * PDR and the SSR notifier are registered in ctrl_probe and call
	 * into this module. They must be torn down here unconditionally:
	 * the child's remove never runs when the child probe failed, and
	 * a notifier left behind oopses in freed module text on the next
	 * LPASS stop/restart.
	 */
	pdr_handle_release(ctrl->pdr);
	qcom_unregister_ssr_notifier(ctrl->notifier, &ctrl->nb);
	...
	if (ctrl->ngd && ctrl->ngd->pdev) {
		dev_info(&pdev->dev, "SPX: unregistering NGD child device for reload\n");
