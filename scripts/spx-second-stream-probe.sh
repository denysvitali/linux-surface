#!/bin/bash
# SPX second-stream probe: does a SECOND stream on the same boot still render?
#
# Mirrors spx-speakers-up.sh power/attach/mixer exactly, but drops the
# "fresh guarded boot" gate so it can measure whether the DSP re-attaches to a
# new ASM session after the first stream was torn down. Read-only evidence is
# the Q6 close counter: write_done>0 means the DSP consumed the second stream.
#
...
if (( EUID == 0 )); then
	echo "FATAL: run as the desktop user, without sudo" >&2
	exit 1
fi
cd "$(dirname "$0")/.."
...
set_ctl() { amixer -c "$CARD" cset name="$1" "$2" >> /tmp/spx-second-mixer.log 2>&1 || { echo "FAILED ctl $1=$2"; exit 1; }; }
...
echo "=== [D] play 5s 440Hz tone (C1/front-right) ==="
TONE_WAV=/tmp/spx-second-tone.wav
...
kmsg_marker() { printf '<6>SPX_2ND_%s_%s\n' "$1" "$RANDOM" | sudo tee /dev/kmsg >/dev/null; }
kmsg_marker tone
timeout -k 2 10 aplay -D "plughw:${CARD},0" --period-size=12000 --buffer-size=48000 "$TONE_WAV" > /tmp/spx-second-aplay.log 2>&1 &
TONE_PID=$!
wait "$TONE_PID" || true
sleep 3
...
echo "=== [F] Q6 stream summary (all streams this boot) ==="
sudo dmesg | grep -E 'SPX ASM stream' | tail -6
