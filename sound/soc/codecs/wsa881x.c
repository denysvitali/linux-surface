...
static int wsa881x_init(struct wsa881x_priv *wsa881x)
{
	...
	if (wsa881x->hw_init && !wsa881x_spx_windows_pa(wsa881x))
		return 0;
...
static int wsa881x_component_probe(struct snd_soc_component *comp)
{
	...
	if (wsa881x->spx_write_only) {
		ret = snd_soc_dapm_add_routes(snd_soc_component_to_dapm(comp), &spx_stream_route, 1);
		if (ret)
			return ret;
		dev_info(comp->dev,
...
static void wsa881x_remove(struct sdw_slave *pdev)
{
